    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <Card>
        ...
        <CardContent className="grid gap-4">
            ...
            {form.status === 'paid' ? (
              <>
                <div className="space-y-1.5">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Cuándo se realizó el pago</p>
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                />
                <p className="text-xs text-muted-foreground">Cuándo debe pagarse</p>
              </div>
            )}
          </div>
          ...
          {form.status === 'paid' && (
            <div className="space-y-1.5">
              <Label htmlFor="due_date">Fecha de Vencimiento (referencia)</Label>
              <Input
                id="due_date"
                type="date"
                value={form.due_date}
                onChange={(e) => handleChange('due_date', e.target.value)}
              />
              <p className="text-xs text-muted-foreground">Fecha original en que debía pagarse (opcional)</p>
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent className="grid gap-4">
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              {form.status === 'paid' && form.plan_id && '✓ Se creará la suscripción automáticamente'}
            </p>
          </div>
