...
import { Plus, X, Loader2, ArrowLeft, ArrowRight, Check, Star, Users } from 'lucide-react'
...
export function NewMatchButton({ competitionId }: Props) {
  ...
  // Step 1 submit: create match, advance to step 2
  async function handleCreateMatch(e: React.FormEvent) {
    ...
    start(async () => {
      try {
        ...
        setStep(2)
        // Lazy-fetch athletes when entering step 2
  ...
  // Step 2 submit: bulk-add athletes to roster
            ...
            {step === 1 && (
              <form onSubmit={handleCreateMatch} className="p-5 space-y-4 overflow-y-auto">
                ...
                <div className="flex gap-3 pt-1">
                  <button type="button" onClick={closeModal} className="flex-1 h-9 rounded-lg border border-input text-sm hover:bg-accent transition-colors">
                    Cancelar
                  </button>
              ...
              <div className="flex flex-col overflow-hidden flex-1">
                <div className="px-5 pt-4 pb-3 border-b border-border space-y-2">
                  <p className="text-xs text-muted-foreground">
                    Cita a los jugadores para esta convocatoria. Marca <span className="font-medium">titulares</span> y <span className="font-medium">suplentes</span>. Puedes saltar este paso y completarla luego.
                  </p>
